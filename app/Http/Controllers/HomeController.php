...
class HomeController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        foreach ($ar as $arr)
        {
            ...
            $d->psize = $arr["psize"];
            $d->pcolor = $arr["pcolor"];
            $p = Product::findOrFail($arr["id"]);

            $d1 = ProductColor::findOrFail($d->psize);
            if($d1->pquantity>0) {
                $total +=  $p->price * $arr["qty"];
                $d1->pquantity -= $arr["qty"];
                $d1->save();
                $d->save();
            }
            else
            {
                $outofstock.=".Some Order out of Stock.Check Order History";
            }
        }
        $order->total = $total;
        if($total>0)
    ...
    public function AddToCart(Request $request )
    {
        ...
        if($product->pquantity >= $request->qty)
        {
            ...
            if(!$found)
            {
                $arr1[$request->psize]= array('id'=>$request->id,'qty'=>$request->qty,'psize'=>$request->psize,'pcolor'=>$request->pcolor);
